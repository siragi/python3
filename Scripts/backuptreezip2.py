...
def process(i, o):
    ...
    try:
        # Decorating following functions with standard messages
        # for func in [shutil.copytree, zippy.createZip, print_diff_files]:
        #     func = wrapmsg(args.verbosity)(func)


        say(
            "'%s', backup: '%s'." % (i, o),
            ("Backing up with permissions and timestamps (using copytree): \n"
             "'%s' --> '%s'" % (i, o)))

        # Decorating with standard messages
        copytree = wrapmsg(args.verbosity)(shutil.copytree)
        copytree(i, o, symlinks=False, ignore=None)

        compcode = print_diff_files(dircmp(i, o))

        createzip = wrapmsg(args.verbosity)(zippy.createZip)
        if compcode == 0:
            os.chdir(os.path.dirname(o))
            zipsource = os.path.basename(o)
            ziparchive = "{}.zip".format(zipsource)
            createzip(zipsource, ziparchive)
        else:
            return compcode
